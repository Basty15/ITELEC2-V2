...
{ 
    ...
    public function sendOtp($otp, $email){
        if($email == NULL){
            ...
            $stmt = $this->runQuerry("SELECT * FROM user WHERE email = :email");
    ...
    public function addAdmin($csrf_token,$username, $email, $password)
    {
        ...
        $stmt = $this->runQuerry('INSERT INTO user (username, email, password) VALUES (:username,:email,:password)');
    ...
    public function adminSignin($email, $password,$csrf_token)
    {
     try{
        ...
        if($stmt->rowCount() == 1 && $userRow['password'] == md5($password)){
            $activity = "Has Successfuly signed in";
            $user_id = $userRow['id'];
    ...
    public function adminSignout()
    {
       ...
       echo "<script>alert('Sign Out Successfuly.'); window.location.href = '../../../';</script>";
            exit;
    }
    ...
    {
      $stmt = $this->runQuerry("INSERT INTO logs (user_id, activity) VALUES (:user_id, :activity)");
